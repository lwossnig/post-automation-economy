...
\hypertarget{carried}{}
\hypertarget{further-results-condensation-speed-leakage-and-the-cost-of-waiting}{%
\section{Further results: condensation speed, leakage, and the cost of
waiting}\label{further-results-condensation-speed-leakage-and-the-cost-of-waiting}}
...
precisely because revenue resists shifting) is the load-bearing
instrument, and the robot tax is a useful complement rather than the
...
infrastructure at home, and a behavioural wealth tax to hold inequality
down, with the proceeds banked in a sovereign fund rather than fully
rebated. The instruments are complements: in the experiments the full
...
\item
  \textbf{A stacked compute rent.} The model bundles the chip-maker and
  the model-owner into a single foreign rentier. A fuller treatment
  would peel a second wedge from the compute input price (a
  \texttt{mu\_frac} twin on the AI-compute stock) held by a distinct
  foreign owner, with its own contestability dial, the experiment-O
  question asked of the compute layer rather than the model layer, where
  the current evidence on accelerator margins cuts against
  commoditisation. Because that rent is embedded in a capital good
  rather than a licence flow, the exercise would also let the model
  price the one instrument the present levers omit, a tariff or usage
  charge on imported compute, and quantify how much of the durable
  surplus the source levy alone leaves uncaptured (the form-of-rent
  qualification in Appendix A.2).
\end{itemize}
...
\hypertarget{appendix-a-assumptions-and-limitations}{%
\section{Appendix A: Assumptions and
limitations}\label{appendix-a-assumptions-and-limitations}}
...
rent that even the reaching instruments miss. We carry the full
two-rentier treatment as an extension (below); the point to register
here is that confining the rent to a single, withholding-reachable
\emph{form}, like confining it to the AI cluster, understates rather
than overstates the problem.
