...
class ParamsV3:
    ...
    # ---- Phase 3 (sandbox): market-priced rent-bearing IP ----
    # Stage 1 is a DIAGNOSTIC ONLY (no feedback): capitalise the rent stream into an
    # IP asset value and report the owner's market-value wealth. In this model the
    # rent is the foreign IP owner's licence fee, not a return on the domestic capital
    # stock, so the object that is worth more than book is the capitalised rent, not a
    # Tobin's q on K. equity_price_on gates later stages (feedback, revaluation account).
    equity_price_on: bool = False
    equity_risk_premium: float = 0.02   # premium over r_required in the discount rate
    # ---- Phase 3 Stage 4 (sandbox): market-clearing equity price ----
    # q is a Tobin's-q price on the equity stock: the equity yield capitalised at the
    # required return (q = yield / r_required), with an optional demand-pressure
    # deviation. Market value of equity is q*K; changes in q are capital gains
    # (revaluations) that feed the consumption wealth effect. market_clearing_on =
    # False keeps q == 1 and is bit-identical to the frozen model. Setting it True
    # with q_portfolio_sensitivity = 0 gives the pure no-arbitrage price; the
    # sensitivity adds a deviation when desired holdings differ from supply.
    market_clearing_on: bool = False
    q_portfolio_sensitivity: float = 0.0   # theta: demand-pressure deviation from fundamental
    q_adjust_speed: float = 0.25           # damping of q toward its target (stability)
...
class HistoryV3:
    ...
    v_ip: list = field(default_factory=list)              # [diagnostic] capitalised rent (IP value)
    own_row_mkt: list = field(default_factory=list)       # [diagnostic] foreign ownership at market value
    nw_sum_market: list = field(default_factory=list)     # Stage 2: market-value net worth of all sectors
    row_nw_market: list = field(default_factory=list)     # Stage 2: RoW net worth incl. the IP
    q_mkt: list = field(default_factory=list)             # Stage 4: market price of equity (Tobin's q)
    cap_gain: list = field(default_factory=list)          # Stage 4: capital gain (revaluation) from dq
...
class ModelV3:
    def __init__(self, p: ParamsV3):
        ...
        self.v_ip = 0.0   # Phase 3 (sandbox): market value of the owner's rent-bearing IP
        self.q_mkt = 1.0  # Phase 3 Stage 4 (sandbox): market price of equity (Tobin's q)
    ...
    def step(self) -> None:
        ...
        if p.two_channel:
            ...
            # competitive capital incomes net of the rent wedge on the cognitive side
            ci_Kr_pot = d["ci_Kr"]
        ...
        # Stage 4: the wealth effect is on the MARKET value of equity (q times book),
        # so capital gains from a rising q lift consumption. q == 1 when the market
        # price is off, recovering the book wealth effect exactly.
        q_w = self.q_mkt if self.p.market_clearing_on else 1.0
        eq_wealth_mkt = eq_wealth * q_w
        ...
        desired_c = np.clip(p.c_income * np.clip(labour_disp, 0, None)
                            ...
                            + p.c_wealth * eq_wealth_mkt, 0, None)
        ...
        X_row = 0.0
        leak = p.trade_leak
        # Stage 3 feedback: when the rent has capitalised into a high market value,
        # reinvesting in dear domestic assets is unattractive, so the owner takes more
        # of the rent out as goods. The valuation raises the effective repatriation,
        # which slows ownership drift (cash leaves rather than buying equity). Routed
        # exactly like the existing trade leak, so the invariants are preserved.
        if p.equity_price_on:
            q_val = 1.0 + self.v_ip / max(self.K, 1e-9)
            leak = float(np.clip(p.trade_leak + (1.0 - 1.0 / q_val), 0.0, 0.95))
        if p.two_channel and leak > 0.0:
            rent_now = self._mu_eff * d["s_c"] * Y                       # rent at realised output
            self._rent_now = float(rent_now)
            keep_rent = max(1.0 - p.dst_ai - p.tax_repat, 0.0)        # owner's share after host tax
            X_row = min(leak * rent_now * keep_rent, I_gross)        # cannot exceed investable residual
        ...
        if p.two_channel:
            ...
            self._dst_rent = p.dst_ai * np.clip(rent_ai, 0, None) * (1.0 - self._rent_shift)  # on recognised rent
        ...
        repat_tax_roy = p.tax_repat * royalty * (1.0 - self._rent_shift)   # withholding on recognised royalty
        ...
        # stock).
        house_flow = (wage_i + int_house + ubi_i + fund_rebate + div_house
        ...
        row_flow = div_row + int_row + royalty - repat_cash - self._dst_rent - self._X_row
        ...
        # ---- Stage 1 diagnostic: capitalise the rent stream into an IP value ----
        # No feedback into the accounting; this only reports what the foreign owner's
        # rent-bearing IP would be worth at market, and how that lifts their share of
        # market-value wealth above book ownership.
        rent_now_d = getattr(self, "_rent_now", 0.0)
        keep_rent = max(1.0 - self.p.dst_ai - self.p.tax_repat, 0.0)   # owner's share after host tax
        net_rent = rent_now_d * keep_rent
        g_smooth = float(np.mean(self.hist.g_rate[-20:])) if self.hist.g_rate else 0.0
        disc = self.p.r_required + self.p.equity_risk_premium
        if net_rent > 0.0:
            v_ip = float(np.clip(net_rent / max(disc - g_smooth, 0.01), 0.0, 60.0 * net_rent))
        else:
            v_ip = 0.0
        self.hist.v_ip.append(v_ip)
        # the capitalised rent is the foreign owner's asset, above book K
        self.hist.own_row_mkt.append((self.eq_row + v_ip) / max(self.K + v_ip, 1e-9))
        # ---- Stage 2: market-value balance sheet (accounting overlay, no feedback) ----
        # The IP is an intangible real asset held by the rest of the world, so the
        # economy's net wealth at market is the book capital stock plus the IP value,
        # and that whole increment sits in the foreign owner's net worth. The book and
        # transactions invariants are untouched; the stock invariant generalises to
        # NW_sum_market = K + v_ip, which holds by construction (it is the book identity
        # plus the same v_ip on both sides), and the test suite checks it each period.
        self.v_ip = v_ip
        # ---- Stage 4: market-clearing equity price (Tobin's q) ----
        # q is the equity yield capitalised at the required return, with an optional
        # demand-pressure deviation, damped for stability. Equity is valued at q times
        # book; the change in q is a capital gain (a revaluation, not a transaction).
        q_old = self.q_mkt
        if self.p.market_clearing_on:
            q_fund = float(np.clip(self.eq_yield_lag / max(self.p.r_required, 1e-6), 0.2, 5.0))
            pressure = float(np.clip(
                (max(self.row_dep, 0.0) + max(self.gov_dep, 0.0)
                 + float(np.clip(self.h_dep, 0.0, None).sum()) - dK) / max(self.K, 1e-9),
                -0.5, 0.5))
            q_target = q_fund * (1.0 + self.p.q_portfolio_sensitivity * pressure)
            self.q_mkt = float(np.clip(q_old + self.p.q_adjust_speed * (q_target - q_old), 0.2, 5.0))
        else:
            self.q_mkt = 1.0
        eq_claims = self.h_eq.sum() + self.eq_state + self.eq_row
        cap_gain = (self.q_mkt - q_old) * eq_claims      # revaluation of the equity stock
        self.hist.q_mkt.append(self.q_mkt)
        self.hist.cap_gain.append(float(cap_gain))
        # market-value balance sheet: equity at q times book, plus the IP, plus deposits
        qe = self.q_mkt
        deposits = float(self.h_dep.sum() + self.gov_dep + self.row_dep + self.firm_dep)
        row_nw_market = qe * self.eq_row + self.row_dep + v_ip
        nw_sum_market = qe * eq_claims + deposits + v_ip
        self.hist.row_nw_market.append(row_nw_market)
        self.hist.nw_sum_market.append(nw_sum_market)
